...
describe("Transaction model", () => {
  const transactionRepositoryMock = {
    findByUserUid: () =>
      Promise.resolve([{ uid: "transaction1" }, { uid: "transaction2" }]),
  };

  describe("given find user by uid", () => {
    test("when user is not informed, then retrun error 500", async () => {
    ...
    test("when user uid is not informed, then retrun error 500", async () => {
      const model = new Transaction();
      model.user = {};
    ...
    test("when user uid is informed, then retrun transactions", async () => {
